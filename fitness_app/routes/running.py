...
def running_page():
    ...
    all_runs = (
        Activity.query.filter_by(user_id=uid, exercise_type_id=r_type)
    ...
    plan = (
        TrainingPlan.query.filter(
            TrainingPlan.user_id == uid, TrainingPlan.name.like("%Running%")
        )
    ...
    goal = (
        UserGoal.query.filter(UserGoal.user_id == uid, UserGoal.goal_type.like("%run%"))
    ...
    all_dates = (
        ...
        .filter(Activity.user_id == uid, Activity.exercise_type_id == r_type)
        .distinct()
    ...
    pace_now = (
        db.session.query(func.avg(Activity.pace_per_km))
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == r_type,
            Activity.date >= monday,
...
def delete_run(activity_id):
    ...
    r = Activity.query.filter_by(
        activity_id=activity_id, user_id=uid, exercise_type_id=get_running_type_id()
    ).first_or_404()
