...
from fitness_app1.extensions import db
from fitness_app1.modele import ( Activity, TrainingPlan, UserGoal,
...
def cycling_page():
    ...
    all_rides = Activity.query.filter_by(
        user_id=uid,
        exercise_type_id=c_type
    ).order_by(Activity.date.desc(), Activity.activity_id.desc()).all()

    #rides this week
    rides_week = db.session.query(func.count(Activity.activity_id))\
        .filter(
    ...
    short_rides = db.session.query(func.count(Activity.activity_id))\
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == c_type,
    ...
    #med ride 10-25
    med_rides = db.session.query(func.count(Activity.activity_id))\
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == c_type,
            Activity.distance_km > 10,
    ...
    #long ride 25-50km
    long_rides = db.session.query(func.count(Activity.activity_id))\
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == c_type,
            Activity.distance_km > 25,
    ...
    xl_rides = db.session.query(func.count(Activity.activity_id))\
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == c_type,
            Activity.distance_km > 50
    ...
    plan = TrainingPlan.query.filter(
        ...
        TrainingPlan.name.like('%Cycling%')
    ...
    #avg speed this week
    speed_now = db.session.query(func.avg(Activity.average_speed_kmh))\
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == c_type,
            Activity.date >= monday,
    ...
    speed_before = db.session.query(func.avg(Activity.average_speed_kmh))\
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == c_type,
    ...
    if speed_now and speed_before and speed_before > 0:
        progress = round(
            ((speed_now - speed_before) / speed_before) * 100
        )
...
def delete_ride(activity_id):
    ...
    r = Activity.query.filter_by(
        ...
        user_id=uid,
        exercise_type_id=get_cycling_type_id()
    ).first_or_404()
