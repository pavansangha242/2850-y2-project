...
def events_page():
    ...
    upcoming = (
        Competition.query.filter(Competition.date >= date.today())
        .order_by(Competition.date)
        .all()
    )
    next_event = upcoming[0] if upcoming else None

    return render_template("events.html", events=upcoming, next_event=next_event)
...
def event_details(competition_id):
    ...
    return render_template(
        "event_details.html", event=competition, is_registered=is_registered
    )
...
def register_event(competition_id):
    ...
    if not existing:
        ...
        db.session.commit()
        flash(
            "Registered successfully! A confirmation has been sent to your email.",
            "success",
        )
...
def event_chat(competition_id):
    ...
    messages = (
        ChatMessage.query.filter_by(competition_id=competition_id)
