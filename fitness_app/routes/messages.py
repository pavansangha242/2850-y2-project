...
def messages():
    ...
    for uid in user_ids:
        ...
        unread = TrainerMessage.query.filter_by(
            sender_id=uid, receiver_id=user.user_id, is_read=False
        ).count()
    ...
    group_subq = (
        db.session.query(ChatMessage.competition_id)
    ...
    for (gid,) in group_subq:
        ...
        latest = (
            ChatMessage.query.filter_by(competition_id=gid)
    ...
    if selected_group_id:
        ...
        if selected_group:
            msgs = (
                ChatMessage.query.filter_by(competition_id=selected_group_id)
    ...
    if selected_contact:
        ...
        TrainerMessage.query.filter_by(
            sender_id=selected_contact.user_id, receiver_id=user.user_id, is_read=False
        ).update({"is_read": True})
        db.session.commit()
        ...
        msgs = (
            TrainerMessage.query.filter(
                or_(
                    and_(
                        TrainerMessage.sender_id == user.user_id,
                        TrainerMessage.receiver_id == selected_contact.user_id,
                    ),
                    and_(
                        TrainerMessage.sender_id == selected_contact.user_id,
                        TrainerMessage.receiver_id == user.user_id,
                    ),
                )
            )
    ...
    selected_profile = (
        TrainerProfile.query.filter_by(user_id=selected_id).first()
        if selected_id
        else None
    )
    ...
    for m in msgs:
        if hasattr(m, "message"):  # TrainerMessage
            author_role = (
                ...
                else (selected_contact.role if selected_contact else "customer")
            )
...
def send_user_message():
    ...
    if trainer_id and message_txt:
        msg = TrainerMessage(
            sender_id=user.user_id, receiver_id=trainer_id, message=message_txt
        )
        db.session.add(msg)
        db.session.commit()
...
def trainer_inbox():
    ...
    for cid in client_ids:
        ...
        latest = (
            TrainerMessage.query.filter(
                or_(
                    and_(
                        TrainerMessage.sender_id == user.user_id,
                        TrainerMessage.receiver_id == cid,
                    ),
                    and_(
                        TrainerMessage.sender_id == cid,
                        TrainerMessage.receiver_id == user.user_id,
                    ),
                )
            )
        ...
        unread = TrainerMessage.query.filter_by(
            sender_id=cid, receiver_id=user.user_id, is_read=False
        ).count()
    ...
    conversations.sort(
        key=lambda x: x["latest"].sent_at if x["latest"] else 0, reverse=True
    )
    ...
    if selected_client:
        # mark messages as read when conversation is opened
        TrainerMessage.query.filter_by(
            sender_id=selected_client.user_id, receiver_id=user.user_id, is_read=False
        ).update({"is_read": True})
        db.session.commit()
        ...
        msgs = (
            TrainerMessage.query.filter(
                or_(
                    and_(
                        TrainerMessage.sender_id == user.user_id,
                        TrainerMessage.receiver_id == selected_client.user_id,
                    ),
                    and_(
                        TrainerMessage.sender_id == selected_client.user_id,
                        TrainerMessage.receiver_id == user.user_id,
                    ),
                )
            )
    ...
    total_unread = TrainerMessage.query.filter_by(
        receiver_id=user.user_id, is_read=False
    ).count()
...
def send_trainer_message():
    ...
    if client_id and message_txt:
        msg = TrainerMessage(
            sender_id=user.user_id, receiver_id=client_id, message=message_txt
        )
        db.session.add(msg)
        db.session.commit()
