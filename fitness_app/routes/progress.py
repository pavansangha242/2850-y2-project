...
def progress_page():
    ...
    if has_data:
        ...
        if selected_sport == "Swimming":
            ...
            pace_data = (
                query.filter(Activity.distance_km > 0, Activity.duration_minutes > 0)
                .order_by(Activity.date.desc())
                .limit(8)
                .all()
            )
        ...
        dist_data = (
            query.filter(Activity.distance_km > 0)
            .order_by(Activity.date.desc())
            .limit(8)
            .all()
        )
        ...
        if selected_sport == "all":
            sport_data = (
                db.session.query(ExerciseType.name, func.sum(Activity.distance_km))
                .join(
                    Activity, Activity.exercise_type_id == ExerciseType.exercise_type_id
                )
                .filter(Activity.user_id == uid, Activity.distance_km > 0)
                .group_by(ExerciseType.name)
                .all()
            )
        ...
        for i in range(7, -1, -1):
            ...
            count = query.filter(
                Activity.date >= week_start, Activity.date <= week_end
            ).count()
        ...
        for i in range(7):
            ...
            km = (
                db.session.query(func.coalesce(func.sum(Activity.distance_km), 0))
                .filter(Activity.user_id == uid, Activity.date == day, *sport_filter)
                .scalar()
                or 0
            )
