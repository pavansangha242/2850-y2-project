...
def _running_stats(user_id, sport_type):
    ...
    total_distance = (
        db.session.query(func.coalesce(func.sum(Activity.distance_km), 0))
        .filter_by(user_id=user_id, exercise_type_id=sport_type.exercise_type_id)
        .scalar()
        or 0
    )
    ...
    total_calories = (
        db.session.query(func.coalesce(func.sum(Activity.calories), 0))
        .filter_by(user_id=user_id, exercise_type_id=sport_type.exercise_type_id)
        .scalar()
        or 0
    )
    ...
    pace_sessions = [
        s for s in reversed(sessions) if s.pace_per_km and s.pace_per_km > 0
    ]
    ...
    chart2_values = [
        round(float(s.distance_km), 2) for s in reversed(sessions) if s.distance_km
    ]
    ...
    personal_bests = [
        {
            "label": f"Best pace: {fmt_pace(best_pace)}",
            "sub": "All time",
            "dot_class": "",
        },
        {
            "label": f"Longest run: {round(longest, 1)} km",
            "sub": "Single session",
            "dot_class": "two",
        },
        {
            "label": f"{total_sessions} sessions total",
            "sub": "All time",
            "dot_class": "green",
        },
        {
            "label": f"{total_distance} km total distance",
            "sub": "All time",
            "dot_class": "amber",
        },
...
def _walking_stats(user_id, sport_type):
    ...
    max_steps = (
        db.session.query(func.max(Activity.steps))
        .filter(
            Activity.user_id == user_id,
            Activity.exercise_type_id == sport_type.exercise_type_id,
            Activity.steps > 0,
    ...
    personal_bests = [
        {
            "label": f"{max_steps:,} steps in one session",
            "sub": "Personal best",
            "dot_class": "",
        },
        {
            "label": f"{round(longest, 1)} km longest walk",
            "sub": "Single session",
            "dot_class": "two",
        },
        {
            "label": f"{total_steps:,} steps total",
            "sub": "All time",
            "dot_class": "green",
        },
        {
            "label": f"{total_distance} km total",
            "sub": "All time",
            "dot_class": "amber",
        },
...
def _cycling_stats(user_id, sport_type):
    ...
    best_speed = (
        db.session.query(func.max(Activity.average_speed_kmh))
        .filter(
            Activity.user_id == user_id,
            Activity.exercise_type_id == sport_type.exercise_type_id,
    ...
    speed_sessions = [
        s for s in reversed(sessions) if s.average_speed_kmh and s.average_speed_kmh > 0
    ]
    ...
    personal_bests = [
        {
            "label": f"Top speed: {round(best_speed, 1)} km/h",
            "sub": "Personal best",
            "dot_class": "",
        },
        {
            "label": f"Longest ride: {round(longest, 1)} km",
            "sub": "Single session",
            "dot_class": "two",
        },
        {
            "label": f"{total_sessions} rides logged",
            "sub": "All time",
            "dot_class": "green",
        },
        {
...
def _swimming_stats(user_id, sport_type):
    ...
    total_laps = (
        db.session.query(func.coalesce(func.sum(Activity.laps), 0))
        .filter_by(user_id=user_id, exercise_type_id=sport_type.exercise_type_id)
        .scalar()
        or 0
    )
    ...
    best_pace_100m = (
        db.session.query(func.min(Activity.pace_per_100m))
        .filter(
            Activity.user_id == user_id,
            Activity.exercise_type_id == sport_type.exercise_type_id,
    ...
    pace_sessions = [
        s for s in reversed(sessions) if s.pace_per_100m and s.pace_per_100m > 0
    ]
    ...
    chart2_labels = [str(s.date) for s in reversed(sessions) if s.distance_km]
    chart2_values = [
        round(float(s.distance_km) * 1000, 0)
    ...
    personal_bests = [
        {
            "label": f"Best pace: {fmt_swim(best_pace_100m)}",
            "sub": "Per 100m",
            "dot_class": "",
        },
        {
            "label": f"Longest swim: {round(longest, 1)} km",
            "sub": "Single session",
            "dot_class": "two",
        },
