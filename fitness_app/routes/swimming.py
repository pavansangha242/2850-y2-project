...
def swimming_page():
    ...
    # get all swims
    all_swims = (
        Activity.query.filter_by(user_id=uid, exercise_type_id=sw_type)
    ...
    plan = (
        TrainingPlan.query.filter(
            TrainingPlan.user_id == uid, TrainingPlan.name.like("%Swimming%")
        )
    ...
    goal = (
        UserGoal.query.filter(
            UserGoal.user_id == uid, UserGoal.goal_type.like("%swim%")
        )
    ...
    all_dates = (
        ...
        .filter(Activity.user_id == uid, Activity.exercise_type_id == sw_type)
        .distinct()
    ...
    # avg pace this wk
    pace_now = (
        db.session.query(func.avg(Activity.pace_per_100m))
        .filter(
            Activity.user_id == uid,
            Activity.exercise_type_id == sw_type,
            Activity.date >= monday,
    ...
    if pace_now and pace_before and pace_before > 0:
        progress = round(((pace_before - pace_now) / pace_before) * 100)
...
def delete_swim(activity_id):
    ...
    sw = Activity.query.filter_by(
        activity_id=activity_id, user_id=uid, exercise_type_id=get_swimming_type_id()
    ).first_or_404()
