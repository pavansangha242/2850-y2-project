...
def seed_database():
    ...
    strength = ExerciseType(name='Strength Training', description='Gym and weight training')
    db.session.add_all([swimming, cycling, running, walking, strength])
    db.session.flush()

    # ---- Users ----
    admin = User(first_name='Michael', last_name='Brown', email='michael@fittrack.com',
                 username='michaelbrown', role='administrator')
