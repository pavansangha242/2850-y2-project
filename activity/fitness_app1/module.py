...
class User(db.Model):
    ...
    role = db.Column(db.String(20), nullable=False)  #customer / pt / admin
...
#extra info for users who are PTs
...
#reviews from clients for their trainer
...
#booking between trainer and client
...
#what the PT can see and do
class PrivacySettings(db.Model):
    __tablename__ = 'privacy_settings'

    privacy_id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id'), unique=True, nullable=False)
    pt_can_see_health_data = db.Column(db.Boolean, default=False)
    pt_can_book_meetings = db.Column(db.Boolean, default=False)
...
class UserGoal(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id'), nullable=False)
    step_target = db.Column(db.Integer, default=0)
    time_exercised_target = db.Column(db.Integer, default=0)
    workouts_per_week_target = db.Column(db.Integer, default=0)
    goal_type = db.Column(db.String(50), default='')
    target_date = db.Column(db.Date, nullable=True)

    def __repr__(self):
        return f'<UserGoal {self.goal_type} for user={self.user_id}>'
...
class Competition(db.Model):
    ...
    #relationships
    registrations = db.relationship('CompetitionRegistration', backref='competition', lazy=True)
    results = db.relationship('CompetitionResult', backref='competition', lazy=True)

    #how many days left til the comp
    @property
...
class CompetitionRegistration(db.Model):
    ...
    def __repr__(self):
        return f'<CompetitionRegistration user={self.user_id} comp={self.competition_id}>'


#result of a user in a comp
class CompetitionResult(db.Model):
    ...
    goal_time = db.Column(db.String(50), default='')
